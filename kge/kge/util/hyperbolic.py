...
def givens_rotations(r, x):
    ...
    givens = givens / torch.norm(givens, p=2, dim=-1, keepdim=True).clamp_min(MIN_NORM)
...
def hyp_distance_(x, v, c, combine:str):
    ...
    if combine == 'sp_':
        norm = torch.norm(v, p=2, dim=-1, keepdim=True).transpose(0, 1)
        xv = x @ v.transpose(0,1) / norm
    elif combine == 'spo':
        norm = torch.norm(v, p=2, dim=-1, keepdim=True)
        xv = torch.sum(x * v / norm, dim=-1, keepdim=True)

    x2 = torch.sum(x * x, dim=-1, keepdim=True)
    gamma = tanh(sqrt_c * norm) / sqrt_c
