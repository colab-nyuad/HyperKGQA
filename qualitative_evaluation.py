...
parser.add_argument(
    "--models", nargs="+", default=["RefH", "RotH", "AttH", "ComplEx", "TransE"], help="Embedding model"
)
...
if __name__ == "__main__":
    ...
    device = 'cuda:0'
    for dim in dims:
        qa_model = torch.load('{}/{}/{}/{}_{}.pt'.format(checkpoint_path, model, dim, model, args.hops))
    ...
    columns = ['{} ({})'.format(model, dim) for dim in dims]
    columns.append('Len')
    ...
    if args.step != 0:
        ...
        rq_frame = rq_frame.iloc[:,:len(args.models)].div(rq_frame.Len, axis=0)
    
    rq_frame = rq_frame.drop(['Len'], axis=1)
    ...
    for model in args.models:
        qa_model = torch.load('{}/{}/{}/{}_{}.pt'.format(checkpoint_path, model, args.dim, model, args.hops))
        qa_model.emb_model.device = device
        qa_model.to(device)
        qa_models.append(qa_model)
